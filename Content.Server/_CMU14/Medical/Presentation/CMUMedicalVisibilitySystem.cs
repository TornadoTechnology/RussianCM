...
public sealed partial class CMUMedicalVisibilitySystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<HumanMedicalStartupEvent>(OnMedicalBodyStartup);
    ...
    private void OnMedicalBodyStartup(ref HumanMedicalStartupEvent args)
    {
        RefreshSubtree(args.Body);
    }
    ...
    private void RefreshEntity(EntityUid uid)
    {
        ...
        var shouldHide = _enabled && IsAttachedToHumanMedicalBody(uid);
    ...
    private bool IsAttachedToHumanMedicalBody(EntityUid uid)
    {
        if (_partQuery.TryGetComponent(uid, out var part) &&
            part.Body is { } partBody &&
            HasComp<HumanMedicalComponent>(partBody))
        {
            return true;
        }
        ...
        if (_organQuery.TryGetComponent(uid, out var organ) &&
            ...
            HasComp<HumanMedicalComponent>(organBody))
        {
            return true;
        }
