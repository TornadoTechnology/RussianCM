...
public sealed partial class CMUAutodocBui : BoundUserInterface
{
    ...
    private Control BuildQueueRow(CMUAutodocBuiState state, CMUAutodocQueueEntry entry)
    {
        ...
        {
            Text = $"{ResolveLabel(entry.SurgeryDisplayName)} - {entry.PartDisplayName}",
            ClipText = true,
            HorizontalExpand = true,
    ...
    {
        return surgery.SurgeryId switch
        {
            nameof(SurgeryProcedureId.SutureWound) => 5f,
            nameof(SurgeryProcedureId.CloseIncision) => 5f,
            nameof(SurgeryProcedureId.RepairFracture) => 12f,
            nameof(SurgeryProcedureId.RepairOrgan) => 12f,
            nameof(SurgeryProcedureId.EyeSurgery) => 12f,
            nameof(SurgeryProcedureId.BrainDamageSurgery) => 12f,
            _ => 8f,
        };
    }
    ...
    {
        return seconds < 60f
            ? Loc.GetString("cmu-autodoc-seconds", ("seconds", Math.Max(1, (int) Math.Ceiling(seconds))))
            : Loc.GetString("cmu-autodoc-minutes", ("minutes", Math.Max(1, (int) Math.Ceiling(seconds / 60f))));
    }
