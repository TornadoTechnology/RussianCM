...
namespace Content.Client._CMU14.Medical.Foundation.Telemetry;
...
public sealed partial class CMUMedicalPerfCommand : IConsoleCommand
{
    ...
    public void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        {
            if (_entities.HasComponent<HumanMedicalComponent>(uid))
                cmuBodies++;
    ...
    private bool IsAttachedCmuInternal(EntityUid uid)
    {
        if (_entities.TryGetComponent(uid, out BodyPartComponent? part) &&
            ...
            _entities.HasComponent<HumanMedicalComponent>(partBody))
        {
            return true;
        }
        ...
        if (_entities.TryGetComponent(uid, out OrganComponent? organ) &&
            ...
            _entities.HasComponent<HumanMedicalComponent>(organBody))
        {
            return true;
        }
